...
#########################################################################
# Output silencer for functions in a library (looking at you openturns) #
#########################################################################
#class FunctionSilencer(object):
#  def __enter__(self):
#    self.stdout = sys.stdout
#    self.saveStdout = copy.copy(sys.stdout)
#    sys.stdout = self
#
...
############################################
# Output savedItems dictionnary to h5 file #
############################################
def DumpObject(SavedItems, fileName="dummy.h5", prefix=''):
  # TODO: Add a check of file existence and some interactive I_O to overwrite or not
  try:
    outputFile = h5py.File(fileName, 'r+')
  except:
    outputFile = h5py.File(fileName, 'w') # Create/Overwrite/Complete the file (Temporary)

  for key in SavedItems:
    newKeyChain = [key]
    newPath = key
    DumpEntry(outputFile, newPath, prefix, SavedItems)
  outputFile.close()

def DumpEntry(outputFile, path, prefix, SavedItems):
  #Find entry
  keyChain = path.split("/")
  entry = SavedItems
  for k in keyChain:
  ...
  if isinstance(entry, dict):
    ...
    outputFile.require_group(realPath)
    for key in entry:
      newPath = path + "/" + key
      DumpEntry(outputFile, newPath, prefix, SavedItems)
  else:
    DumpSingularEntry(outputFile, path, prefix, SavedItems)

def DumpSingularEntry(outputFile, path, prefix, SavedItems):
  #Find entry
  keyChain = path.split("/")
  entry = SavedItems
  for k in keyChain:
  ...
  if prefix == '':
    ...
    realPath = prefix + '/' + path
  #try:
  outputFile.create_dataset(realPath, data=entry)
  #except:
  #  del outputFile[realPath]
  #  outputFile.create_dataset(realPath, data=entry)
...
#####################################
def DictFromH5(myFile, prefix=''):
  willExit = False
  try:
    myH5 = h5py.File(myFile, 'r')
  except:
    willExit = True
  if willExit:
    prt('', 'red', True)
    prt('Utils/I_O.py/DictFromH5', 'red', True)
    prt('The file ' + myFile + ' does not exit or is not readable as an h5 file.', 'red', True)
    prt('Exiting', 'red', True)
    sys.exit()
  # Initialization
  myDict = {}
  ...
  if prefix == '':
    for key in myH5:
      myDict[key] = GetDataFromKey(key, myH5)
  else:
    for key in myH5[prefix]:
      realKey = prefix + '/' + key
      myDict[key] = GetDataFromKey(realKey, myH5)
  return myDict

def GetDataFromKey(myKeyPath, myH5):
  # Recurrence
  isDataset = True
  try:
    myH5.require_group(myKeyPath)
    isDataset = False
  except:
    pass
  if isDataset:
    try:
      return myH5[myKeyPath][:]
    except:
      return myH5[myKeyPath][()]
  else:
    newDict={}
    for key in myH5[myKeyPath]:
      newKeyPath = myKeyPath + "/" + key
      newDict[key] = GetDataFromKey(newKeyPath, myH5)
    return newDict
    

###########################
# Get object type in file #
###########################
def GetObjectType(objectFile, objectPath):
  myFile  = h5py.File(objectFile, 'r')
  objType = myFile[objectPath + '/ObjectType'][()]
  myFile.close()
  return objType


################################################################
# Progress bar to check procedures advancement (e.g. sampling) #
################################################################
class ProgBar:
  ######################
  #     Properties     #
  ...
  #     Methods     #
  ###################
  def __init__(self, message, maxNumber):
    self.isProgBar  = False
    self.bar        = None
    try:
      from progress.bar import Bar
      self.isProgBar = True
    except:
      prt('Please consider installing the module "progress" to use the progress bar.', 'red', True)
    self.maxNumber = maxNumber
    if self.isProgBar:
      self.bar = Bar(message, max=self.maxNumber) #, suffix='%(percent)d%%')
...
#############################################################
# Input file handler module from Matteo Gelain (EM2C/ONERA) #
...
class InputHandler:
            ...
            if line[0] != '-->':
              for word in line:
                ...
                if typ == 'char': 
                  ...
                  try:
                    ...
                    if word == '[]':
                      ...
                      print('Input for key ' + keyName + ' in block ' + self.currentBlock + ' is not an integer as specified.')
                  ...
                  try:
                    ...
                    if word == '[]':
                      ...
                      print('Input for key ' + keyName + ' in block ' + self.currentBlock + ' is not a real as specified.')
                      print('For an empty list, please write "[]".')
                      print('Stopping.')
                      sys.exit()
                elif typ == 'logical':
                  if word == 'True':
                    ...
                    if word == '[]':
                      ...
                      print('Input for key ' + keyName + ' in block ' + self.currentBlock + ' should be either True or False.')
                      print('For an empty list, please write "[]".')
                      print('Got ' + word + ' instead.')
                      print('Stopping.')
                      sys.exit()
                else:
                  print('Type for key '+ keyName + ' in block ' + self.currentBlock + ' seems not to be recognised.')
                ...
                if len(line) > 1:
                  ...
                  print('No values for key ' + keyName + ' in block ' + self.currentBlock + ' were found.')
                  print('Stopping.')
                  sys.exit()
              return self.Value
              ...
              if len(line) < 2:
                ...
                if len(line) == 2: # If there is only one reference
                  ...
                  if len(path) == 2:
                    ...
                    prt('', 'red', True)
                    prt('Utils/I_O.py/InputHandler/GetValueFromKey', 'red', True)
                    prt('Error: ' + line[1] + ' is not a valid reference. Valid format is "blockID/(subBlockID/)specifier"', 'red', True)
                    prt('Exiting', 'red', True)
                    sys.exit()
                else:
                  myValue = []
  ...
  def ReplaceFromSpecifier(self, key, value, **kwargs):
    ...
    if not check:
      ...
      if 'subBlockName' in self__dict__.keys():
