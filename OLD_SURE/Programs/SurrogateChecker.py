...
if modFile != "":
  if mySur.dimension == 1:
    ...
    xMin = xOriSample[:,0].min()
    xMax = xOriSample[:,0].max()
    yMin = xOriSample[:,1].min()
    yMax = xOriSample[:,1].max()
    nPt = 100
    ...
    ax.scatter(xOriSample[:129,0], xOriSample[:129,1], xOriEval[:129], marker='x', s=40, color='r', label='Original samples') #DEBUG
    ax.scatter(xValSample[:129,0], xValSample[:129,1], xValEval[:129], marker='o', s=40, color='k', label='Validation samples') #DEBUG
