...
if isReference:
  ...
  xRefEval = xRefEval[xRefEval!=0.0]
  xRefEval = xRefEval[xRefEval!=1.0]
...
if isReference and surOnRef:
  ...
  ax1.scatter(xRefEval, xRefSurEval, color='k', alpha = 1.0, marker=',', s=1)#, label='Reference samples')
#if surOnOri:
...
if modFile != "":
  if mySur.dimension == 1:
    ...
    ax.scatter(xOriSample[:,0], xOriSample[:,1], np.log(xOriEval[:]), marker='x', s=40, color='k', label='Original samples') #DEBUG
    ax.scatter(xValSample[:,0], xValSample[:,1], np.log(xValEval[:]), marker='x', s=40, color='r', label='Validation samples') #DEBUG
    #ax.view_init(azim=100, elev=9)
    ax.view_init(azim=0, elev=90)
    #ax.view_init(azim=-80, elev=9)
    ax.set_ylabel(r'$\hat{T}$', labelpad=15)
    ax.set_xlabel(r'$\xi_{12}$', labelpad=15)
...
if isReference:
  ...
  if refKDE:
    ...
    n, bins, patches = ax.hist(np.log(xRefEval), nColumn, density=True, facecolor='dimgrey', alpha=0.33, label='PDF (reference)')
  if surKDE:
  ...
  #ax.set_xlim([np.log(xRefEval.min()), np.log(xRefEval.max())])
