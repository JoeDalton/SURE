...
from SurrogateModelling.KrigingTools.Autocorellation_functions  import *
from SurrogateModelling.KrigingTools.optimisationTools          import *
from SurrogateModelling.PCE                                     import *
from Utils.I_O import prt


fileName = 'SurrogateModelling/MyKRiging.py'
...
class PCK_model(SM):
  ...
  def __init__(self, af, x, y, distribution, PCEdeg=2, givenTheta=None, verbosity=0, **kwargs):
    ...
    for i in range(len(xPCEEval)):
        xPCEEval[i] = myPCE.Evaluate(x[i,:])[0]

    if givenTheta is None:
      self.OK = OK_model(af, x, y-xPCEEval, ID='OK', optiStrat=self.optiStrat, optiTarget=self.optiTarget, nugget=self.nugget, verbosity=self.verbosity-1)
      print(OK.theta)
    else:
