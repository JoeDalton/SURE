...
def GenInterpFile_Axi(workDir, meshRelPath, interpRelPath, xlim, rlim, tlim):
  ...
  print "Interpolation mesh contains %i nodes" % (resx*resr*rest)
  ...
  for it in range(rest):
    for ir in range(resr):
      for ix in range(resx):
        uvw.append([xq[ix], rq[ir] * math.cos(tq[it]), rq[ir] * math.sin(tq[it])])
  uvw = np.array(uvw)
  ...
  header = np.array([['resx', 'resr', 'rest', 'xlim', 'rlim', 'tlim', 'isAxiSym'], [resx, resr, rest, xlim, rlim, tlim, True]], dtype=object)
