...
def PlotProfile_Z(workDir, D, variable, label, xAx, xNX, interpRelPath, solRelPath=None, axial=False):
  ...
  if axial:
    ...
    for solutionfile in os.listdir(workDir):
      if solutionfile.endswith(".h5") and solutionfile.startswith(FILEPREFIX):
        ...
        z = cio.read_avbp_solution(os.path.join(os.path.expanduser(workDir), solutionfile), 'z_out')
        print "Interpolating"
        ...
        for it in range(rest):
          ...
          xZtemp.append(zq[imin:imax])
        xQtemp = np.array(xQtemp)
        xZtemp = np.array(xZtemp)
        mean = np.zeros(resx * resr)
        meanz = np.zeros(resx * resr)

        for ip in range(resx * resr):
          mean[ip] = np.sum(xQtemp[:, ip]) / rest
          meanz[ip] = np.sum(xZtemp[:, ip]) / rest

        Q = np.reshape(mean, (resr, resx))
        QZ = np.reshape(meanz, (resr, resx))

        Q[np.isnan(Q)] = 0
        QZ[np.isnan(QZ)] = 0

        ax.plot(QZ[0,:], Q[0,:], label=label)
    ...
    for i in range(len(xNX)):
      ...
      for j in range(resr):
        ...
        znterp = interp1d(xq, QZ[j,:], kind='cubic')
        profile.append(interp(normAxialDistance*D))
        zrofile.append(znterp(normAxialDistance*D))
      ax.plot(zrofile, profile, label=label)
      ax.legend(fontsize=12)
      counter +=1
...
def MasterPlotProfile_Z(workDir, D, variable, label, xAx, xNX, interpRelPath, solRelPath=None):
  # Set general path to solution files and interpolation base file
  FILEPREFIX = 'av_sol'
  ...
  for i in range(1,nPlot+1):
    ...
    for j in range(resr):
      ...
      profile.append(interp(normAxialDistance*D))
      zrofile.append(znterp(normAxialDistance*D))
    #ax.plot(rq, zrofile, label=label)
    ax.plot(zrofile, profile, label=label)
    ax.legend(fontsize=12)
    counter +=1
